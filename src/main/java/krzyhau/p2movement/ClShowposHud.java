...
public class ClShowposHud {
    ...
    private static boolean canDraw() {
        ...
        if (!boots.isOf(ModRegister.LONG_FALL_BOOTS)) {
            return false;
        }

        if(EnchantmentHelper.getLevel(ModRegister.CL_SHOWPOS_ENCHANTMENT,boots) == 0) {
            return false;
        }
    ...
    private static String[] getShowposString() {
        ...
        Vec3d ang = new Vec3d(player.getPitch(), (player.getYaw()-180.0)%360.0+180.0, 0);
        

        Vec3d vel = player.getVelocity().multiply(1.0 / Portal2Movement.MOVE_SCALAR);
